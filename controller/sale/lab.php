...
class ControllerSaleLab extends Controller {

	private $error = array();

	public function index() {

		$this->load->language('sale/lab');

		$this->document->title = $this->language->get('heading_title');

		$this->load->model('sale/lab');
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_sale_lab->editOrder($this->request->get['order_id'], $this->request->post);

			$this->model_sale_lab->addOrderHistory($this->request->get['order_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && ($this->validateDelete())) {
			foreach ($this->request->post['selected'] as $order_id) {
				$this->model_sale_lab->deleteOrder($order_id);
			}
			...
			$this->redirect(HTTPS_SERVER . 'index.php?route=sale/lab&token=' . $this->session->data['token'] . $url);
		}
	...
	public function verifypackingslipaddresses() {
		...
		if (isset($this->request->post['selected'])) {
			...
			foreach ($all_orders as $one_order){
				if($first_time_loop){
					$first_address = $this->model_sale_lab->getShipAddressFromOrder($one_order)->row['order_shipping_address'];
	...
	public function createpackingslip() {
		...
		if (isset($this->request->post['selected'])) {
			...
			foreach($order_list as $single_order_id) {
				$matched_orders = '';
				$order_primary = $this->model_sale_lab->getOrder($single_order_id);
				$order_status_primary = $order_primary['order_status_id'];

				$order_address_primary = $this->model_sale_lab->getShipAddressFromOrder($single_order_id)->row['order_shipping_address'];
				if ($order_status_primary == '130') {

					foreach($order_list as $reference_order_id) {

						$order_secondary = $this->model_sale_lab->getOrder($reference_order_id);
						$order_status_secondary = $order_secondary['order_status_id'];

						$order_address_secondary = $this->model_sale_lab->getShipAddressFromOrder($reference_order_id)->row['order_shipping_address'];

						if ($order_address_primary == $order_address_secondary) {

							$matched_orders .= $reference_order_id . ",";

							$this->model_sale_lab->updateOrderStatusShipped($reference_order_id);
					...
					$success = $this->model_sale_lab->insertPackingSlip($matched_orders, date("Y-m-d"), date("Y-m-d"), "ICS", $order_address_primary);

					if ($success) {
						$packing_slip_ids .= $this->db->getLastId() . ',';

						$this->model_sale_lab->updateOrderStatusShipped($single_order_id);
			...
			$this->model_sale_lab->quickbooksQueue($packing_slip_ids);


			$this->redirect(HTTPS_SERVER . 'index.php?route=sale/lab/viewpackingslip&packing_slip_ids=' . $packing_slip_ids . '&token=' . $this->session->data['token']);
	...
	public function viewpackingslip() {
		...
		if (isset($this->request->get['packing_slip_ids'])) {
			...
			foreach ($packing_slip_ids as $packing_slip_id) {
				...
				$packing_list_data = $this->model_sale_lab->getPackingSlip($packing_slip_id);

				$packing_lists[] = $packing_list_data[0];

				//updating the packing list numbers in the table
				$this->model_sale_lab->updateOrderPackingList($packing_slip_id);
				...
				if (!isset($this->request->get['view'])) {
					...
					$this->model_sale_lab->updateOrderHistory($packing_slip_id,$_SESSION['user_id']);
				}
			...
			foreach($packing_lists as  $packing_list){
				...
				$packing_list_clinics = $this->model_sale_lab->getPackingSlipClinics($packing_list['id']);
				...
				foreach($packing_list_clinics as $packing_list_clinic){
					...
					$packing_list_clinic_name_temp = $this->model_sale_lab->getClinicName($packing_list_clinic['clinic_id']);
					...
					$packing_list_clinic_orders = $this->model_sale_lab->getPackingSlipClinicsOrders($packing_list['id'],$packing_list_clinic['clinic_id']);
			...
			$this->template = 'sale/lab_packing_slip.tpl';
			$this->response->setOutput($this->render(TRUE), $this->config->get('config_compression'));

		} else {

			$this->redirect(HTTPS_SERVER . 'index.php?route=sale/lab/update&token=' . $this->session->data['token']);
		}
	...
	private function getList() {
		...
		$this->document->breadcrumbs[] = array(
				'href' => HTTPS_SERVER . 'index.php?route=sale/lab&token=' . $this->session->data['token'] . $url,
				'text' => $this->language->get('heading_title'),
				'separator' => ' :: '
		);

		$this->data['invoice'] = HTTPS_SERVER . 'index.php?route=sale/lab/invoice&token=' . $this->session->data['token'];
		$this->data['insert'] = HTTPS_SERVER . 'index.php?route=sale/lab/insert&token=' . $this->session->data['token'] . $url;
		$this->data['delete'] = HTTPS_SERVER . 'index.php?route=sale/lab/delete&token=' . $this->session->data['token'] . $url;
		$this->data['packingslip'] = HTTPS_SERVER . 'index.php?route=sale/lab/createpackingslip&token=' . $this->session->data['token'] . $url;
		...
		$results = $this->model_sale_lab->getOrders($data);

		$order_total = $this->model_sale_lab->getTotalOrders($data);
		$newResults = $results;
		...
		foreach ($newResults as $result) {
			...
			if (stristr($result['user_group_step_display'], '"' . $result['order_status_id'] . '"')) {
				$action[] = array(
						...
						'href' => HTTPS_SERVER . 'index.php?route=sale/lab/update&token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url
				);
			} else {
				$action[] = array(
						'text' => 'View',
						'href' => HTTPS_SERVER . 'index.php?route=sale/lab/update&token=' . $this->session->data['token'] . '&v=1&order_id=' . $result['order_id'] . $url
				);
			}
		...
		$this->data['sort_order_id'] = HTTPS_SERVER . 'index.php?route=sale/lab&sort=order_id&token=' . $this->session->data['token'] . $url;
		$this->data['sort_clinic_name'] = HTTPS_SERVER . 'index.php?route=sale/lab&sort=clinic_name&token=' . $this->session->data['token'] . $url;
		$this->data['sort_patient_name'] = HTTPS_SERVER . 'index.php?route=sale/lab&sort=patient_name&token=' . $this->session->data['token'] . $url;
		$this->data['sort_status'] = HTTPS_SERVER . 'index.php?route=sale/lab&sort=status&token=' . $this->session->data['token'] . $url;
		$this->data['sort_date_added'] = HTTPS_SERVER . 'index.php?route=sale/lab&sort=order_date_added&token=' . $this->session->data['token'] . $url;
		// Mod Delivery Date
		$this->data['sort_deliverydate'] = HTTPS_SERVER . 'index.php?route=sale/lab&token=' . $this->session->data['token'] . '&sort=order_deliverydate' . $url;
		$this->data['deliverydate_status'] = $this->config->get('deliverydate_status');
		// End:Mod Delivery Date
		$this->data['sort_total'] = HTTPS_SERVER . 'index.php?route=sale/lab&sort=order_total&token=' . $this->session->data['token'] . $url;
		...
		$pagination->url = HTTPS_SERVER . 'index.php?route=sale/lab&token=' . $this->session->data['token'] . $url . '&page={page}';
		...
		$this->template = 'sale/lab_list.tpl';
		$this->children = array(
				'common/header',
				'common/footer'
		);
	...
	public function generate() {
		...
		if (isset($this->request->get['order_id'])) {
			$json['invoice_id'] = $this->model_sale_lab->generateInvoiceId($this->request->get['order_id']);
		}
	...
	public function history()
	{
		...
		if (!$this->user->hasPermission('modify', 'sale/lab')) {
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			$order_info = $this->model_sale_lab->getOrder($order_id);
			...
			if ($order_info) {
				...
				$products = $this->model_sale_lab->getOrderProducts($order_id);

				foreach ($products as $product) {
					$option_data = array();

					$options = $this->model_sale_lab->getOrderOptions($order_id, $product['order_product_id']);
				...
				$total_data = $this->model_sale_lab->getOrderTotals($order_id);
		...
		$this->template = 'sale/lab_invoice.tpl';
	...
	public function address() {
		...
		if (isset($this->request->get['type']) && $this->request->get['type'] == 'shipping') {
			$this->model_sale_lab->updateShippingAddress($order_id, $this->request->post);
		} elseif (isset($this->request->get['type']) && $this->request->get['type'] == 'payment') {
			$this->model_sale_lab->updatePaymentAddress($order_id, $this->request->post);
		}
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if($patient_id==''){
				...
				$verify_patients = $this->model_sale_lab->verifyPatients($this->request->post);
				...
				else
				{
					...
					$this->model_sale_lab->addOrder($this->request->post);
	...
	private function getForm() {
		...
		if (!isset($this->request->get['product_id'])) {
			$this->data['action'] = HTTPS_SERVER . 'index.php?route=sale/lab/insert&token=' . $this->session->data['token'] . $url;
		} else {
			$this->data['action'] = HTTPS_SERVER . 'index.php?route=sale/lab/update&token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url;
		}

		$this->data['cancel'] = HTTPS_SERVER . 'index.php?route=sale/lab&token=' . $this->session->data['token'] . $url;

		$this->data['token'] = $this->session->data['token'];

		if (isset($this->request->get['order_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$order_info = $this->model_sale_lab->getOrder($this->request->get['order_id']);
		...
		*/
		...
		if (isset($this->request->get['order_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$clinic_verify = $this->model_sale_lab->verifyClinics($order_info['clinic_id']);
...

			{
				...
				$pricing_values = $this->model_sale_lab->getPricingValues($order_info['order_values_37'],$order_info['clinic_id']);

				$pricing_values_tax_rates = $this->model_sale_lab->getPricingItemTaxRate();
				...
				if (isset($this->request->get['order_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
					...
					$item_name = $this->model_sale_lab->getItemName($order_info['order_values_37']);
...
// 					}
					...
					if ($order_info['clinic_id'] == 1){
						...
						$this->model_sale_lab->updateOrderDescription($this->request->get['order_id'],$order_description);
					...
					$this->model_sale_lab->updateUnitPrice($this->request->get['order_id'],$order_unit_pair_cost/$order_info['order_quantity']);
					...
					$this->model_sale_lab->updateOrderTotals($this->request->get['order_id'],$order_total_including_tax);
		...
		$this->template = 'sale/lab_form.tpl';
		$this->children = array(
				'common/header',
				'common/footer'
		);
		...
		if (isset($this->request->post['order_date_needed'])){
			...
			if(isset($this->request->get['order_id']))
			{
			 // getting the current rush date of the order to be inspected
				$existing_date = $this->model_sale_lab->checkCurrentRushDate($this->request->get['order_id']);
			...
			if(isset($this->request->get['order_id']))
			{
			 // getting  the calculated delivery date stored in the records
				$existing_delivery_date = $this->model_sale_lab->checkdeliveryDate($this->request->get['order_id']);
	...
	public function checkPatientRecords(){
		...
		echo $this->model_sale_lab->verifyPatientRecords($first_name,$last_name,$birth_date,$clinic_id);
