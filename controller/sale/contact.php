...
class ControllerSaleContact extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			// All clinics by group
			if (isset($this->request->post['group'])) {
				switch ($this->request->post['group']) {
					case 'newsletter':
						$results = $this->model_sale_clinic->getClinicsByNewsletter();
					
						foreach ($results as $result) {
							$emails[$result['clinic_id']] = $result['email'];
						}
						break;
					case 'clinic':
						$results = $this->model_sale_clinic->getClinics();
			...
			// All clinics by name/email
			if (isset($this->request->post['to']) && $this->request->post['to']) {					
				foreach ($this->request->post['to'] as $clinic_id) {
					$clinic_info = $this->model_sale_clinic->getClinic($clinic_id);
					
					if ($clinic_info) {
						$emails[] = $clinic_info['email'];
					}
				}
			}
			
			// All clinics by product
			if (isset($this->request->post['product'])) {
				foreach ($this->request->post['product'] as $product_id) {
					$results = $this->model_sale_clinic->getClinicsByProduct($product_id);
			...
			foreach ($this->request->post['to'] as $clinic_id) {
				...
				if ($clinic_info) {
					$this->data['clinics'][] = array(
						...
						'name'        => $clinic_info['patient_firstname'] . ' ' . $clinic_info['patient_lastname'] . ' (' . $clinic_info['email'] . ')'
					);
				}
			}
		}
	...
	public function clinics() {
		...
		if (isset($this->request->get['keyword']) && $this->request->get['keyword']) {
			$results = $this->model_sale_clinic->getClinicsByKeyword($this->request->get['keyword']);
		
			foreach ($results as $result) {
				$clinic_data[] = array(
					'clinic_id' => $result['clinic_id'],
					'name'        => $result['patient_firstname'] . ' ' . $result['patient_lastname'] . ' (' . $result['email'] . ')'
				);
			}
		}
		...
		$this->response->setOutput(Json::encode($clinic_data));
	}
