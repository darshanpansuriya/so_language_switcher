...
class ControllerUserUserPermission extends Controller {
	...
	private function getList() {
		...
		foreach ($results as $result) {
			...
			$this->data['user_groups'][] = array(
				...
				'user_group_step_full' => $result['user_group_step_full'],
				'user_group_step_display' => $result['user_group_step_display'],
		...
		$this->data['sort_name'] = HTTPS_SERVER . 'index.php?route=user/user_permission&token=' . $this->session->data['token'] . '&sort=user_group_name' . $url;
	...
	private function getForm() {
		...
		$this->data['tab_general'] = $this->language->get('tab_general');
 		...
 		if (isset($this->error['user_group_name'])) {
			$this->data['error_user_group_name'] = $this->error['user_group_name'];
		} else {
			$this->data['error_user_group_name'] = '';
		}
		...
		if (isset($this->request->post['user_group_name'])) {
			$this->data['user_group_name'] = $this->request->post['user_group_name'];
		} elseif (isset($user_group_info)) {
			$this->data['user_group_name'] = $user_group_info['user_group_name'];
		} else {
			$this->data['user_group_name'] = '';
		}
		
		if (isset($this->request->post['user_group_step_full'])) {
			$this->data['user_group_step_full'] = $this->request->post['user_group_step_full'];
		} elseif (isset($user_group_info)) {
			$this->data['user_group_step_full'] = $user_group_info['user_group_step_full'];
		} else {
			$this->data['user_group_step_full'] = '';
		}
		
		if (isset($this->request->post['user_group_step_display'])) {
			$this->data['user_group_step_display'] = $this->request->post['user_group_step_display'];
		} elseif (isset($user_group_info)) {
			$this->data['user_group_step_display'] = $user_group_info['user_group_step_display'];
		} else {
			$this->data['user_group_step_display'] = '';
		}
		...
		if (isset($this->request->post['user_group_permission'])) {
			$this->data['access'] = $this->request->post['user_group_permission']['access'];
		} elseif (isset($user_group_info['user_group_permission']['access'])) {
			$this->data['access'] = $user_group_info['user_group_permission']['access'];
		} else { 
			$this->data['access'] = array();
		}

		if (isset($this->request->post['user_group_permission'])) {
			$this->data['modify'] = $this->request->post['user_group_permission']['modify'];
		} elseif (isset($user_group_info['user_group_permission']['modify'])) {
			$this->data['modify'] = $user_group_info['user_group_permission']['modify'];
		} else { 
			$this->data['modify'] = array();
		}

		if (isset($this->request->post['user_group_step_display'])) {
			$this->data['display'] = $this->request->post['user_group_step_display']['display'];
		} elseif (isset($user_group_info['user_group_step_display']['display'])) {
			$this->data['display'] = $user_group_info['user_group_step_display']['display'];
		} else { 
		...
		if (isset($this->request->post['user_group_step_full'])) {
			$this->data['full'] = $this->request->post['user_group_step_full']['full'];
		} elseif (isset($user_group_info['user_group_step_full']['full'])) {
			$this->data['full'] = $user_group_info['user_group_step_full']['full'];
		} else { 
	...
	private function validateForm() {
		...
		if ((strlen(utf8_decode($this->request->post['user_group_name'])) < 3) || (strlen(utf8_decode($this->request->post['user_group_name'])) > 64)) {
			$this->error['user_group_name'] = $this->language->get('error_user_group_name');
		}
