...
/*
//BOF vovan options images
            if(isset($this->request->post['show_full_sized']) && (int)$this->request->post['show_full_sized']) {
                $this->response->setOutput($this->model_tool_image->preview_src($this->request->post['image'])); // show original sized image
            } else {
			    $this->response->setOutput($this->model_tool_image->resize($this->request->post['image'], 100, 100));
            }
//EOF vovan options images
*/
			$this->response->setOutput($this->model_tool_image->resize($this->request->post['image'], 100, 100));
		}
	}
	...
	public function create() {
		...
		if (!isset($json['error'])) {	
			...
			chmod($directory . '/' . str_replace('../', '', $this->request->post['name']), 0777);
