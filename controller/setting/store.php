...
class ControllerSettingStore extends Controller {
	...
	public function getForm() { 
		...
		$this->load->model('sale/clinic_group');
		
		$this->data['clinic_groups'] = $this->model_sale_clinic_group->getClinicGroups();
		
		if (isset($this->request->post['clinic_group_id'])) {
			$this->data['clinic_group_id'] = $this->request->post['clinic_group_id'];
		} elseif (isset($store_info)) {
			$this->data['clinic_group_id'] = $store_info['clinic_group_id'];			
		} else {
			$this->data['clinic_group_id'] = '';
		}
		
		if (isset($this->request->post['clinic_price'])) {
			$this->data['clinic_price'] = $this->request->post['clinic_price'];
		} elseif (isset($store_info)) {
			$this->data['clinic_price'] = $store_info['clinic_price'];			
		} else {
			$this->data['clinic_price'] = '';
		}
		
		if (isset($this->request->post['clinic_approval'])) {
			$this->data['clinic_approval'] = $this->request->post['clinic_approval'];
		} elseif (isset($store_info)) {
			$this->data['clinic_approval'] = $store_info['clinic_approval'];			
		} else {
			$this->data['clinic_approval'] = '';
		}
