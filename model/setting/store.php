...
class ModelSettingStore extends Model {
	...
	public function getStoreDescriptions($store_id) {
		...
		$query = $this->db->query("SELECT * FROM store_description WHERE store_id = '" . (int)$store_id . "'");
	...
	public function getTotalStoresByLanguage($language) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM store WHERE language = '" . $this->db->escape($language) . "'");
		
		return $query->row['total'];		
	}
	
	public function getTotalStoresByCurrency($currency) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM store WHERE currency = '" . $this->db->escape($currency) . "'");
		
		return $query->row['total'];		
	}
	
	public function getTotalStoresByCountryId($country_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM store WHERE country_id = '" . (int)$country_id . "'");
		
		return $query->row['total'];		
	}
	
	public function getTotalStoresByZoneId($zone_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM store WHERE zone_id = '" . (int)$zone_id . "'");
		
		return $query->row['total'];		
	}
	
	public function getTotalStoresByClinicGroupId($clinic_group_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM store WHERE clinic_group_id = '" . (int)$clinic_group_id . "'");
		
		return $query->row['total'];		
	}	
	
	public function getTotalStoresByInformationId($information_id) {
      	$account_query = $this->db->query("SELECT COUNT(*) AS total FROM store WHERE account_id = '" . (int)$information_id . "'");
      	
		$checkout_query = $this->db->query("SELECT COUNT(*) AS total FROM store WHERE checkout_id = '" . (int)$information_id . "'");
		
		return ($account_query->row['total'] + $checkout_query->row['total']);
	}
	
	public function getTotalStoresByOrderStatusId($order_status_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM store WHERE order_status_id = '" . (int)$order_status_id . "'");
