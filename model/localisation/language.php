...
class ModelLocalisationLanguage extends Model {
	public function addLanguage($data) {
		...
		// Coupon
		$query = $this->db->query("SELECT * FROM coupon_description WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");

		foreach ($query->rows as $coupon) {
			$this->db->query("INSERT INTO coupon_description SET coupon_id = '" . (int)$coupon['coupon_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($coupon['name']) . "', description = '" . $this->db->escape($coupon['description']) . "'");
		}
		...
		foreach ($query->rows as $product) {
			$this->db->query("INSERT INTO product_description SET product_id = '" . (int)$product['product_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($product['name']) . "', meta_description= '" . $this->db->escape($product['meta_description']) . "', desc_upgrade= '" . $this->db->escape($product['desc_upgrade']) . "', description = '" . $this->db->escape($product['description']) . "'");
		}

		$this->cache->delete('product');

		// Product Option
		$query = $this->db->query("SELECT * FROM product_option_description WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");

		foreach ($query->rows as $product_option) {
			$this->db->query("INSERT INTO product_option_description SET product_option_id = '" . (int)$product_option['product_option_id'] . "', language_id = '" . (int)$language_id . "', product_id = '" . (int)$product_option['product_id'] . "', name = '" . $this->db->escape($product_option['name']) . "'");
		}
		
		// Product Option Value
		$query = $this->db->query("SELECT * FROM product_option_value_description WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");

		foreach ($query->rows as $product_option_value) {
			$this->db->query("INSERT INTO product_option_value_description SET product_option_value_id = '" . (int)$product_option_value['product_option_value_id'] . "', language_id = '" . (int)$language_id . "', product_id = '" . (int)$product_option_value['product_id'] . "', name = '" . $this->db->escape($product_option_value['name']) . "'");
		}
		...
		// Store
		$query = $this->db->query("SELECT * FROM store_description WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");

		foreach ($query->rows as $store) {
			$this->db->query("INSERT INTO store_description SET store_id = '" . (int)$store['store_id'] . "', language_id = '" . (int)$language_id . "', description = '" . $this->db->escape($store['description']) . "'");
		}
	...
	public function deleteLanguage($language_id) {
		...
		$this->db->query("DELETE FROM store_description WHERE language_id = '" . (int)$language_id . "'");
		...
		$this->db->query("DELETE FROM coupon_description WHERE language_id = '" . (int)$language_id . "'");
		...
		$this->db->query("DELETE FROM product_option_description WHERE language_id = '" . (int)$language_id . "'");
		$this->db->query("DELETE FROM product_option_value_description WHERE language_id = '" . (int)$language_id . "'");
