	...
	public function updateCurrencies() {
		if (extension_loaded('curl')) {
			...
			foreach ($lines as $line) {
				...
				if ((float)$value) {
					$this->db->query("UPDATE currency SET value = '" . (float)$value . "', date_modified = NOW() WHERE code = '" . $this->db->escape($currency) . "'");
				}
			}
			
			$this->db->query("UPDATE currency SET value = '1.00000', date_modified = NOW() WHERE code = '" . $this->db->escape($this->config->get('config_currency')) . "'");
