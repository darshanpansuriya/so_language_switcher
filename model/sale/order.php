...
class ModelSaleOrder extends Model {
	...
	public function addOrder($data) {
		...
		if (isset($data['order_values_60']) && $data['order_values_60'] != '254' && isset($_SESSION['s_order_id']) && $_SESSION['s_order_id']) {
			exec('rm -rf hc_scripts/plupload/uploads/' . $_SESSION['s_order_id']);
		}
		$o = "INSERT INTO `order` SET order_date_added = '".$current_date_time."'";
		if ($data['patient_id'])
			$op = "UPDATE patient SET patient_date_modified = '".$current_date_time."', patient_status = '1'";
		else
			$op = "INSERT INTO patient SET patient_date_added = '".$current_date_time."',patient_status = 1";
		...
		// for shipping addresss
		$format_address = isset($data['format_address']) ? $data['format_address'] :'';
		$format_street = isset($data['format_street']) ? $data['format_street'] :'';
		$format_city = isset($data['format_city']) ? $data['format_city'] :'';
		$format_province = isset($data['format_province']) ? $data['format_province'] :'';
		$format_postal_code = isset($data['format_postal_code']) ? $data['format_postal_code'] :'';
		$order_shipping_address = $format_address.', '.$format_street.', '.$format_city.', '.$format_province.', '.$format_postal_code;
		$o .= ", order_shipping_address = '" . $this->db->escape($order_shipping_address) . "'";
		...
		$this->db->query($op);
		$fill_with_poron=0;
		if(isset($data['fill_with_poron'])) {
			$fill_with_poron=$data['fill_with_poron'];
		}
		//p(__LINE__);
		if (!$data['patient_id'])
			$patient_id = $this->db->getLastId();
		$o .= ",shoes_width='". htmlspecialchars($data['shoes_width'], ENT_QUOTES) ."', fill_with_poron='".$fill_with_poron."', order_patient_id = '$patient_id', order_originaldelivery = '" . $this->hc_functions->ch_date($this->db->escape(strip_tags($data['order_deliverydate']))) . "'";
		$this->db->query($o);
		...
		order_history_order_status_id = '" . $data['order_status_id'] . "',
				order_history_date = '".$current_date_time."',
	...
	public function editOrder($order_id, $data) {
		...
		// defining the query
		$order_value_q="select order_values_order_id from order_values  WHERE order_values_order_id = '".$data['order_id']."'";
		$result = mysql_query($order_value_q);
		$order_value = mysql_fetch_row($result);
		if(!is_array($order_value))
		{
			$ov = "INSERT INTO order_values SET order_values_order_id = '".$data['order_id']."'";
			$this->db->query($ov);
		}
		$s = "SELECT order_values_7,order_status_id FROM `order_values` ov LEFT JOIN `order` o ON o.order_id = ov.order_values_order_id WHERE order_values_order_id = '".$data['order_id'] . "'";
		...
		//p($data,__LINE__.__FILE__);
		$o = "UPDATE `order` SET order_date_modified = '".$current_date_time."'";
		$op = "UPDATE `patient` SET patient_date_modified = '".$current_date_time."'";
		//p($data);
		foreach ($data as $key => $val) {
			if (substr($key, 0, 12) == 'order_values') {
				$exist = $this->db->query("SHOW COLUMNS FROM `order_values` LIKE '$key'");
		...
		$o.= ", shoes_width = '". htmlspecialchars($data['shoes_width'], ENT_QUOTES)."', fill_with_poron = '".$fill_with_poron."'";
		...
		if (isset($data['history_updated']) && $data['history_updated']) {
			...
			order_history_order_id = '$order_id',
			order_history_order_status_id = '" . $data['order_status_id'] . "',
					order_history_date_added = '".$current_date_time."',
					order_history_comment = '" . $this->db->escape($data['order_history_comment']) . "'";
			$this->db->query($s);
		}
		/********************************************************************/
	...
	public function addOrderHistory($order_id, $data) {
		 ...
		 */
		// james - there is where the update is to delivery date
		if ((int) $data['order_status_id'] == 30) {

			date_default_timezone_set('America/New_York');

			$shippingDate = date("Y-m-d",strtotime("+5 weekdays"));

			if (date("N",strtotime($day)) == 6 || date("N",strtotime($day)) == 7) {
				$shippingDate = date("Y-m-d",strtotime($day ." +" . (8 - date("N",strtotime($day))) . " days"));
			}

			$order_deliverydate = $shippingDate;
			...
			if (isset($data['order_values_rush_75']) && $data['order_values_rush_75'] != '')
			{
		...
		$this->db->query("UPDATE `order` SET order_status_id = '" . (int) $data['order_status_id'] . "', order_date_modified = '".$current_date_time."' WHERE order_id = '" . (int) $order_id . "'");
		...
		foreach ($packing_list_ids as $packing_list_id)
		{
			...
			else
			{
				...
				foreach ($packing_orders as $packing_order)
				{
					...
					$clinic_id = '';
					if(is_object($order_primary_clinic)){
					...
					foreach ($packing_orders as $packing_second_order)
					{
						...
						if(is_object($order_secondary_clinic)){
						$clinic_secondary_id = $order_secondary_clinic->clinic_id;
						}
						$clinic_primary_id_new=0;
						$clinic_secondary_id_new=0;
						if ($clinic_primary_id_new == $clinic_secondary_id_new)
						{
	...
	public function updateOrderHistory($packing_slip_id) {
		...
		$packing_slips_orders = $query->row['orders'];
		foreach(explode(',',$packing_slips_orders) as $order_id){
			$this->db->query("INSERT INTO `order_history` SET order_history_order_status_id = 150,order_history_user_id = ".$_SESSION['user_id']. ",order_history_comment= 'Packing slip generated',order_history_date= '".$current_date_time."' ,order_history_order_id = " . $order_id);
		}
	}
	...
	public function getOrders($data = array()) {
		...
		if (isset($data['filter_status']) && !is_null($data['filter_status'])) {
			...
			$sql .= "
					AND o.order_status_id > '0' AND o.order_status_id != '180'";
		}
		...
		if (isset($data['filter_date_added']) && !is_null($data['filter_date_added'])  && $data['filter_date_added'] != '0000-00-00' && !isset($data['filter_date_added_2']) && is_null($data['filter_date_added_2'])) {
			$sql .= "
					AND DATE(order_date_added) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
		}else if(isset($data['filter_date_added']) && !is_null($data['filter_date_added']) && isset($data['filter_date_added_2']) && !is_null($data['filter_date_added_2'])){
			$sql .= "
					AND order_date_added BETWEEN '" . $this->db->escape($data['filter_date_added'].' 00:00:00') . "' AND '" . $this->db->escape($data['filter_date_added_2'].' 23:59:59') . "' ";
		}

		if (isset($data['filter_deliverydate']) && !is_null($data['filter_deliverydate']) && $data['filter_deliverydate'] != '0000-00-00' && !isset($data['filter_deliverydate_2']) && is_null($data['filter_deliverydate_2'])) {
			$sql .= "
		...
		}else if(isset($data['filter_deliverydate']) && !is_null($data['filter_deliverydate']) && isset($data['filter_deliverydate_2']) && !is_null($data['filter_deliverydate_2'])){
				$sql .= "
					AND (DATE(order_deliverydate) BETWEEN DATE('" . $this->db->escape($data['filter_deliverydate'].' 00:00:00') . "') AND  DATE('" . $this->db->escape($data['filter_deliverydate_2'].' 23:59:59') . "')";
			$sql.="OR ((DATE(order_date_needed) BETWEEN DATE('" . $this->db->escape($data['filter_deliverydate'].' 00:00:00') . "')  AND DATE('" . $this->db->escape($data['filter_deliverydate_2'].' 23:59:59') . "')) AND ov.order_values_rush_75='342'))";
		}
	...
	public function getOrdersByAdminFilter($data = array()) {
		$user_id = $this->session->data['user_id'];
		$user_group_string = $this->user->getUGstr();
		$sql = "SELECT SQL_CALC_FOUND_ROWS DISTINCT  o.order_id,clinic_name,
				CONCAT(p.patient_firstname, ' ',
		...
		if (in_array('11', explode(',', $user_group_string))) {
			...
			$sql .= "
					AND o.order_status_id IN (30, 35, 40, 50, 60, 70, 80, 81, 82, 83, 84, 86, 88, 90, 100, 110, 120)";
