...
function do_ortho(id,shell,material,thickness) {
	...
	datastr = 'id='+id + '&s='+shell+'&m='+material+'&t='+thickness+ '&r=' + $('input[name=order_values_37]:checked').val();
...
//			alert($('#order_values_7').val() + '-' + $('#order_values_53').val()+'-' + $('#order_values_54').val());
//					alert("done...");
		}
	});
}
...
function do_mods(id,grind,material,thickness,flange,hl,hr) {
	...
	datastr = 'id='+id + '&g='+grind+'&m='+material+'&t='+thickness+ '&f=' + flange + '&hl=' + hl+'&hr=' + hr+ '&r=' + ortho;
...
//			alert(html);
			$("#div_" + id).html(html);
			if(!ortho == '185' && (! $('#order_values_12').val() || $('#order_values_12').val() == 61))
...
//				 $('#order_values_11').val('55');
			...
			if (ortho =='181' || ortho == '219') {
				$('input[name=order_values_left_61]').attr('readonly','readonly');
				$('input[name=order_values_left_61]').attr('readonly','readonly');
			}
...
function show_hide_file_frame() {
	...
	if ($("input[name=order_values_60]:checked").val() =='254') {
		...
		if ($("input[name=order_values_60]:checked").val() =='253')
			$("#file_warn").html("Please reference the Scan File # in the notes below");
	}
...
function do_equal_sides(elx,vid,vzero){
	if (elx == 'order_values_left_' + vid && $('#order_values_right_' + vid).val() == vzero) {
		$('#order_values_right_' + vid).val($('#' + elx).val())
	} else if (elx == 'order_values_right_' + vid && $('#order_values_left_' + vid).val() == vzero) {
...
function do_check_both(elx,vid){
	...
	if (elx == 'order_values_left_' + vid) {
		$('#order_values_right_' + vid).attr('checked',$('#order_values_left_' + vid).attr('checked'));
	}
}
...
function do_ortho_shell_type(obj) {
	...
	if ($(obj).attr('checked')) {
		if (obj.value == '316') $('#order_values_7').val('41').trigger('change');
		else if (obj.value == '317') $('#order_values_7').val('42').trigger('change');
	}
}
...
function put_address() {
	$('#order_shipping_address').val($('#ship_addressx').val().replace(/\,\ \ /g,'\n').replace('&quot;','"'));
        
}
/**
 * this function is written to disable the hallux field if the ortho type is custom direct milled
 */
...
function do_hallux(){
	...
	if($('input[name=order_values_37]:checked').val() == '316'){
		...
		$('input[name=order_values_hallux_rigidus_splint_with_reinforcement_67]').attr('disabled','disabled');
		...
		$('input[name=order_values_hallux_rigidus_splint_with_reinforcement_67]').removeAttr('disabled');
		
	}
	
}
/*
 * this function is written to display the information on the additions tab on orders page
 */
...
function postings_disable(){	
	if ($('input[name=order_values_55]:checked').val() == '238') {
		$('.disabled').find('input, textarea, button, select').attr('disabled','disabled');
		...
		$('.disabled').find('input, textarea, button, select').removeAttr('disabled');
		...
		{			
			...
			if($('input[name=order_values_21]:checked').val()=='109') {				
				$('#order_values_21_109').attr('checked','checked').trigger('change');
			}
			if($('input[name=order_values_21]:checked').val()=='110') {			
				$('#order_values_21_110').attr('checked','checked').trigger('change');
			}
...
function rf_extrinsic() {
	...
	$('.div_22_112').show(300);
	$('.div_22_113').show(300);
	if ($('#order_values_7').val() == '41' && $('input[name=order_values_21]:checked').val()== '109') {
		$('#order_values_22_112').attr('checked','checked').trigger('change');
	} else if ($('#order_values_7').val() == '42' && $('input[name=order_values_21]:checked').val()== '109'){
		$('#order_values_22_112').removeAttr('checked').trigger('change');
		$('#order_values_22_113').attr('checked','checked').trigger('change');
		$('.div_22_112').hide(300);
	}
...
function rf_intrinsic() {
	...
	$('.div_22_112').show(300);
	$('.div_22_113').show(300); 
	if ($('#order_values_7').val() == '41' && $('input[name=order_values_21]:checked').val()== '110') {
		$('#order_values_22_112').attr('checked','checked').trigger('change');
		$('.div_22_113').hide(300);
	} else if ($('#order_values_7').val() == '42' && $('input[name=order_values_21]:checked').val()== '110'){
		$('#order_values_22_112').removeAttr('checked').trigger('change');
		$('#order_values_22_113').removeAttr('checked').trigger('change');
		$('.div_22').hide(300);
	}
...
function rf_degrees() {
	if ($('#order_values_7').val() == '42' && $('input[name=order_values_21]:checked').val()== '110') {
		// no p
//	} else if($('input[name=order_values_21]:checked').val()==undefined || $('input[name=order_values_22]:checked').val()==undefined) {
	} else if($('input[name=order_values_21]:checked').val()==undefined || $('input[name=order_values_21]:checked').val()== 108) {
//		alert('Select Post Type and Post material'); return false;
		alert('Select Post Type'); return false;
	}
}
...
function ff_extrinsic() {
	...
	$('.div_70_328').show(300);
	$('.div_71').show(300);
	if ($('#order_values_7').val() == '41' && $('input[name=order_values_69]:checked').val()== '325') {
		$('#order_values_71_330').attr('checked','checked').trigger('change');
		$('#order_values_70_328').attr('checked','checked').trigger('change');
		$('.div_70_327').hide(300);
	} else if ($('#order_values_7').val() == '42' && $('input[name=order_values_69]:checked').val()== '325'){
		$('#order_values_70_328').attr('checked','checked').trigger('change');
		$('.div_70_327').hide(300);
	}
...
function ff_intrinsic() {
	...
	if ($('#order_values_7').val() == '41' && $('input[name=order_values_69]:checked').val()== '326') {
		$('#order_values_70_327').attr('checked','checked').trigger('change');
		$('.div_70_328').hide(300);
	} else if ($('#order_values_7').val() == '42' && $('input[name=order_values_69]:checked').val()== '326'){
...
function ff_degrees() {
	if ($('#order_values_7').val() == '42' && $('input[name=order_values_69]:checked').val()== '326') {
		// no p
//	} else if($('input[name=order_values_69]:checked').val()==undefined || $('input[name=order_values_70]:checked').val()==undefined) {
	} else if($('input[name=order_values_69]:checked').val()==undefined ||$('input[name=order_values_69]:checked').val()== 324 ) {
//		alert('Select Post Type and Post material'); return false;
		alert('Select Post Type'); return false;
	}
}
...
function do_midlayer(id) {
	if ($('#order_values_30').val()=='162' || $('#order_values_35').val() == undefined ) {
		if (id == 141 ) $('#order_values_30').val(164).trigger('change');
		else if(id == 142)  $('#order_values_30').val(166).trigger('change');
		else if(id == 143)  $('#order_values_30').val(168).trigger('change');
	}
}
...
//	alert(idx);
	def[idx] = valx;
/*
	idsx = [24,26,25,27,28,29,30,31,32,33,34,63,62,35];
	for (i in idsx) {
		id = idsx[i];
		def[id] = $('#order_values_'+id).val();
		if (idx == id) def[id] = valx;
		vals[id] = ''; // all is available
	} */
	idsx=[];
	if (idx == '316' || idx == '317') {
	...
	if (mid == 24) {
		...
		if($('#order_values_24').val() == 117) {
			def[26] = 133; vals[26] = 133;
			def[25] = 134;  vals[25] = 134;
			def[27] = 140; vals[27] = 140;
		...
		if ($('#order_values_24').val() == 118) {
			id=26; vals[id] = '129,130,135,131,132,136';
			def[id] = ($('#order_values_' + id).val()==133 ? 129 :  $('#order_values_' + id).val());
			$('#order_values_' + id).val(def[id]);
			
			if ($('#order_values_26').val() == 129) {
				id=25; vals[id] = '125,126,127';
				def[id] = $('#order_values_' + id).val()==134 ? 126 :  $('#order_values_' + id).val();
				$('#order_values_' + id).val(def[id]);
				...
				id=25; vals[id] = '125,126';
				def[id] = $('#order_values_' + id).val()==125 ? 125:126;
				$('#order_values_' + id).val(def[id]);
			}
		} else if($('#order_values_24').val() == 120 || $('#order_values_24').val() == 354) {
			id=26; vals[id] = '129'; def[id] = 129;
			id=25; vals[id] = '128'; def[id] = 128;
		} else if($('#order_values_24').val() == 124) {
			id=26; vals[id] = '129,139';
			...
			id=26; vals[id] = '129,137,138'; 
			if($('#order_values_' + id).val() != 137 && $('#order_values_' + id).val() != 138) def[id] = 129;
			id=25; vals[id] = '128';
			def[id] = 128;
			$('#order_values_' + id).val(def[id]);
			...
			def[id] = 129;
			$('#order_values_' + id).val(def[id]);
			id=25; vals[id] = '128,126';
			if($('#order_values_' + id).val() != 128) def[id] = 126;
			...
			if ($('#order_values_26').val() == 129) {
				...
				id=25; vals[id] = '126';
				if($('#order_values_' + id).val()!=125 && $('#order_values_' + id).val()!=358) def[id] = 126;
				$('#order_values_' + id).val(def[id]);
			}
		}
	} else if (mid == 28) {
		...
		if ($('#order_values_28').val() != 144) {
			if ($('#order_values_27').val() == 140) {
				id=30;def[id] = 163; vals[id] = 163;
			} else if ($('#order_values_27').val() == 141) {
				id=30;def[id] = 164; vals[id] = 164;
			} else if($('#order_values_27').val() == 142) {
				id=30;def[id] = 164; vals[id] = 164;
			} else if($('#order_values_27').val() == 143) {
				id=30;def[id] = 168; vals[id] = 168;
			}
		}
		
		if($('#order_values_28').val() == 144) {	
			def[29]=148;def[30]=162;	
			vals[29] = 148; vals[30] = 162;
		} else if($('#order_values_28').val() == 146) {
			id=29; vals[id] = '149,150';
		...
		if ($('#order_values_31').val() != 170) {
			if ($('#order_values_27').val() == 140) {
				id=33;def[id] = 163; vals[id] = 163;
			} else if ($('#order_values_27').val() == 141) {
				id=33;def[id] = 165; vals[id] = 165;
			} else if($('#order_values_27').val() == 142) {
				id=33;def[id] = 167; vals[id] = '165,167';
			} else if($('#order_values_27').val() == 143) {
				id=33;def[id] = 169; vals[id] = '165,167,169';
			}
		}
		...
		if($('#order_values_31').val() == 170) {	
			def[32]=174;def[33]=163;	
			vals[32]=174;vals[33]=163;	
		} else if($('#order_values_31').val() == 172) {
			id=32; vals[id] = '175,176';
		...
		if ($('#order_values_34').val() != 152) {
			if ($('#order_values_27').val() == 140) {
				id=35; def[id] = 155; vals[id] = 155;				
			} else if ($('#order_values_27').val() == 141) {
				id=35; def[id] = 156; vals[id] = '156,159';
			} else if($('#order_values_27').val() == 142) {
//				alert('589 hc_order_form.js');
				id=35; def[id] = 157; vals[id] = '157,160';
			} else if($('#order_values_27').val() == 143) {
				id=35; def[id] = 158; vals[id] = '158,161';
			}
		}
		...
		if($('#order_values_34').val() == 152) {	
			def[63]=353;def[62]=257;def[35]=155;
			vals[63]=353;vals[62]=257;vals[35]=155;
		} else if($('#order_values_34').val() == 153) {
			id=63; vals[id] = '255,258';
...
//				alert('646 hc_order_form.js');
	...
	}  else if (mid != 30 && mid != 33 && mid != 35) { // below is the same as idx == 27; 27 is also changed with ortho type page
//		alert('675 hc_orde_form.js');
		do_covers(99,$('#order_values_27').val(),27);
	} 
	for (i in idsx) {
		id = idsx[i]; 
		if(vals[id] === undefined) {vals[id]='';} // '' means all available values
		cover_ajax(id,vals[id],def[id]);
	}
...
//		alert(1);
		$('#order_values_'+leftright+'_'+dir).append(new Option("Neutral",dval));
		$('#order_values_'+other+'_'+dir).append(new Option("Neutral",dval));
		$('#order_values_'+leftright+'_'+dir).val(dval);
		$('#order_values_'+leftright+'_'+dir).attr('disabled','disabled');
	} else {
		$('#order_values_'+leftright+'_'+dir).removeAttr('disabled');
		$('#order_values_'+other+'_'+ dir).removeAttr('disabled');
		$('#order_values_'+leftright+'_'+dir+" option[value='"+dval+"']").remove();
		$('#order_values_'+other+'_'+dir+" option[value='"+dval+"']").remove();
...
// vrecurs == 0 means add functions to checkboxes, except onready function use 1.

	if ($('#order_values_' + vwhat).val() == vval) {
		$('#order_values_left_' + vcheck).attr('checked',false).attr('disabled','disabled');
		$('#order_values_right_' + vcheck).attr('checked',false).attr('disabled','disabled');
	} else {
...
function mods_change(thid){
	...
	if (thid == 11) {
		if (valx == 60) {
			$('#order_values_left_61').attr('checked',false).attr('readonly','readonly');
			$('#order_values_right_61').attr('checked',false).attr('readonly','readonly');
			
			id=14; vals = 274; def=274;
		} else {
			$('#order_values_left_61').attr('readonly',false).attr('disabled',false).attr('checked',true);
			$('#order_values_right_61').attr('readonly',false).attr('disabled',false).attr('checked',true);
			id=14; 
			curval = $('#order_values_' + id).val();
			if(valx == 55) {
				vals = '76,77,300,399'; def = (curval == 274 ? def = 77 : curval);
			} else if(valx == 58) {
				vals = '77,300,399';  def = (curval == 274 ? def = 77 : curval);
			} else if(valx == 56) {
				vals = '76'; def = (curval == 274 ? def = 76 : curval);
			} else if(valx == 59) {
				vals = '76,77';  def = (curval == 274 ? def = 77 : curval);;
			} else if(valx == 57) {
				vals = '400,76,77';  def = (curval == 274 ? def = 77 : curval);
			}else if(valx == 410) {
				vals = '76,77';  def = (curval == 274 ? def = 77 : curval);;
			}
		...
		if (valx == 70) {
			$('#order_values_left_64').attr('checked',false).attr('readonly','readonly');
			$('#order_values_right_64').attr('checked',false).attr('readonly','readonly');
		} else {
			$('#order_values_left_64').attr('readonly',false).attr('disabled',false).attr('checked',true);
			$('#order_values_right_64').attr('readonly',false).attr('disabled',false).attr('checked',true);
		}
