...
<div class="h1float">
    <h1>Lab <strong>Report</strong></h1>
</div>

<div class="buttons">
    <span style="float:left !important;">
    <a id="batch_save_status" class="button e batch_save_component" name="batch_save_status" style="display:none !important;"><p></p><span>Save</span></a> &nbsp;
    ...
    <a id="hc_save_status" class="button e" name="hc_save_status" style="display:none !important;"><p></p><span>Save Status</span></a> &nbsp;

    <?php // } ?>

    <!-- Added by Mary on 07/25/2018 -->
    <!--  -->
    <?php if (in_array($user_group_string, array('1','1'))) { ?>
    <span class="sp_hc_filter_order">
    <a onclick="filterByAdmin()" class="button f"><p></p><span>Filter Order</span></a>
    <?php } ?> &nbsp;


    <?php if (in_array($user_group_string, array('1','11'))) { ?>
    <span class="sp_hc_copy_order">
        <a id="hc_copy_order" class="button o"><p></p><span>Copy Order</span></a>
        &nbsp; </span>
    <a href="index.php?route=sale/order/insert&token=<?php echo $token;?>" class="button a"><p></p><span>Add Order</span></a>
    <?php } ?> &nbsp;
...
<? if(!in_array($user_group_string, array('11'))): ?>
    <span class="sp_hc_print_label">
        <a onclick="$('#form').attr('action', 'hc_scripts/print_labels.php'); $('#form').attr('target', '_self'); $('#form').submit();$('#form').attr('action', '');" class="button pr"><p></p><span>Create <strong>Labels</strong></span></a> &nbsp; </span>
<? endif; ?>
        <? if(in_array($user_group_string, array('1'))): ?>
    <span class="sp_hc_delete_order">
        <a onclick="$('#form').attr('action', '<?php echo $delete; ?>'); $('#form').attr('target', '_self'); $('#form').submit();" class="button d"><p></p><span><?php echo $button_delete; ?> <strong>Order</strong></span></a> &nbsp; </span>
<span class="sp_hc_print_order">
        <a onclick="printOrder()" class="button pr"><p></p><span>Print <strong>Order</strong></span></a> &nbsp; </span>
   <span class="sp_hc_download_order">
        <a onclick="downloadOrder()" class="button e"><p></p><span>Download <strong>Order</strong></span></a> &nbsp; </span>

   <span class="sp_hc_create_packing_slip">
        <a onclick="handlePackSlip();" class="button pr"><p></p><span>Print Pack List</span></a> &nbsp; </span>

    <? endif; ?>

    <a class="button d" onclick="clear_filter();"><p></p><span>Clear</span></a>
    ...
    <table class="list">
        <thead>
            <tr>
                ...
                <td class="right">
                    <a href="<?php echo $sort_order_id; ?>"<?php echo ($sort == 'order_id'?' class="'. strtolower($order).'"':'')?>>Order ID</a></td>
                <td class="left">
                    <a href="<?php echo $sort_clinic_name; ?>"<?php echo ($sort == 'clinic_name'?' class="'. strtolower($order).'"':'')?>>Clinic Name</a></td>

                <td class="left">
                    <a href="<?php echo $sort_patient_name; ?>" 
                       <?php if ($sort == 'patient_name') echo 'class="'.strtolower($order).'"';?>>Patient Name</a></td>
                ...
                <td class="left"><?php if ($sort == 'order_deliverydate') { ?>
                    <a href="<?php echo $sort_deliverydate; ?>" class="<?php echo strtolower($order); ?>"><?php //echo $column_deliverydate; ?>Shipping Date</a>
                    <?php } else { ?>
                    <a href="<?php echo $sort_deliverydate; ?>"><?php //echo $column_deliverydate; ?>Shipping Date</a>
                    <?php } ?></td>
